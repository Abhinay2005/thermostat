...
struct ThermometerDialView: View {
    ...
    var body: some View {
        ZStack {
            ...
            // MARK: Inner Dial
            Circle()
                .fill(LinearGradient([Color("Inner Dial 1"), Color("Inner Dial2")]))
                .frame(width: innerDialSize, height: innerDialSize)
            
            // MARK: Temperature Setpoint
            Circle()
                .fill(LinearGradient([Color("Temperature Setpoint 1"), Color("Temperatute Setpoint 2")]))
